...
type StoreInfo = { name: string; tel: string };

/**
 * 拠点の表示情報。両店舗の電話番号を本文に併記する(要件 C章)。
 * 電話番号は環境変数から取得する(サーバー稼働中に .env で差し替え可能とするため、
 * モジュール読み込み時ではなく呼び出し時に評価する)。
 * NOTE: 電話番号は原典に実値があるが本リポジトリからは取得できないため暫定値。
 *       本番投入前に実際の店舗電話番号を STORE_TEL_HYUGA / STORE_TEL_NOBEOKA に設定すること。
 */
function resolveStoreInfo(): Record<PlaceCode, StoreInfo> {
  return {
...
function buildBody(
  input: ReservationMailInput,
  stores: Record<PlaceCode, StoreInfo>,
): string {
  const store = stores[input.place];
  const both = stores;
  return [
    `${input.name} 様`,
    "",
